...
fn bench_block_hash_sizes(c: &mut Criterion) {
    let mut ctx = GpuContext::new_sync().expect("GPU 初始化失败");
    let hasher = BlockHashComputer::new(&mut ctx).expect("创建失败");

    let mut group = c.benchmark_group("block_hash_sizes");

    for (w, h) in [(16u32, 16u32), (32, 32)] {
        ...
        group.bench_with_input(BenchmarkId::new("cpu", &size), &batch, |b, imgs| {
            b.iter(|| {
                for img in imgs {
                    let _ = hash_reference::block_hash(black_box(img), w, h);
                }
            })
        });
    }
...
fn bench_block_hash_batch(c: &mut Criterion) {
    let mut ctx = GpuContext::new_sync().expect("GPU 初始化失败");
    let hasher = BlockHashComputer::new(&mut ctx).expect("创建失败");

    let mut group = c.benchmark_group("block_hash_batch");
    ...
    for batch_size in [1, 10, 100, 1000] {
        ...
        group.bench_with_input(BenchmarkId::new("cpu", batch_size), &batch, |b, imgs| {
            b.iter(|| {
                for img in imgs {
                    let _ = hash_reference::block_hash(black_box(img), 16, 16);
                }
            })
        });
    }
...
fn bench_block_hash_workgroup(c: &mut Criterion) {
    let mut group = c.benchmark_group("block_hash_workgroup");
    ...
    for wg_size in [64u32, 128, 256, 512] {
        let mut ctx = GpuContext::new_sync().expect("GPU 初始化失败");
        let hasher =
            BlockHashComputer::with_workgroup_size(&mut ctx, [wg_size, 1, 1]).expect("创建失败");
...
criterion_group!(benches, bench_block_hash_sizes, bench_block_hash_batch, bench_block_hash_workgroup);
