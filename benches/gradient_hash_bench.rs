...
fn bench_gradient_hash_sizes(c: &mut Criterion) {
    let mut ctx = GpuContext::new_sync().expect("GPU 初始化失败");
    let hasher = GradientHashComputer::new(&mut ctx).expect("创建失败");

    let mut group = c.benchmark_group("gradient_hash_sizes");

    for (w, h) in [(8u32, 9u32), (16, 17), (32, 33)] {
        let size = format!("{}x{}", w, h);
        let image = test_data::horizontal_gradient_image(w, h);
        ...
        group.bench_with_input(BenchmarkId::new("cpu", &size), &batch, |b, imgs| {
            b.iter(|| {
                for img in imgs {
                    let _ = hash_reference::gradient_hash(black_box(img), w, h);
                }
            })
        });
    }
...
fn bench_gradient_hash_batch(c: &mut Criterion) {
    let mut ctx = GpuContext::new_sync().expect("GPU 初始化失败");
    let hasher = GradientHashComputer::new(&mut ctx).expect("创建失败");

    let mut group = c.benchmark_group("gradient_hash_batch");
    ...
    for batch_size in [1, 10, 100, 1000] {
        ...
        group.bench_with_input(BenchmarkId::new("cpu", batch_size), &batch, |b, imgs| {
            b.iter(|| {
                for img in imgs {
                    let _ = hash_reference::gradient_hash(black_box(img), 8, 9);
                }
            })
        });
    }
...
fn bench_gradient_hash_workgroup(c: &mut Criterion) {
    let mut group = c.benchmark_group("gradient_hash_workgroup");
    ...
    for wg_size in [64u32, 128, 256, 512] {
        let mut ctx = GpuContext::new_sync().expect("GPU 初始化失败");
        let hasher =
            GradientHashComputer::with_workgroup_size(&mut ctx, [wg_size, 1, 1]).expect("创建失败");
...
criterion_group!(benches, bench_gradient_hash_sizes, bench_gradient_hash_batch, bench_gradient_hash_workgroup);
