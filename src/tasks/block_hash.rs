...
impl BlockHashComputer {
    /// 创建 Block Hash 计算器，使用默认 workgroup_size [256, 1, 1]。
    pub fn new(ctx: &mut GpuContext) -> Result<Self, GpuError> {
        Self::with_workgroup_size(ctx, DEFAULT_WORKGROUP_SIZE)
    }

    /// 创建 Block Hash 计算器，指定 workgroup_size。
    pub fn with_workgroup_size(
        ctx: &mut GpuContext,
        workgroup_size: [u32; 3],
    ) -> Result<Self, GpuError> {
        let pipeline = ctx.get_or_create_pipeline(BLOCK_HASH_WGSL, workgroup_size)?;
        Ok(Self {
            pipeline,
            workgroup_size,
        })
...
impl PerceptualHashComputer for BlockHashComputer {
    fn compute(
        &self,
        ctx: &GpuContext,
        images: &[Vec<u8>],
