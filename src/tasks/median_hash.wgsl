// Median Hash 着色器：从输入缓冲区读取预计算的中位数，与像素比较设置哈希位。
//
// 输入缓冲区布局（每张图）：[pixels_per_image 个 u32 像素, 1 个 u32 中位数]
// 步长 stride = pixels_per_image + 1，中位数位于 pixels[base + pixels_per_image]。
//
// 中位数由 Rust 端预计算后追加到像素数据末尾，避免 GPU 端 O(N²) 循环
// （原方案每线程遍历全部像素构建 256-bin 直方图，hash_size=32 时 DX12 循环展开
// 导致 register pressure 过大，着色器编译静默失败）。
