...
impl PdqHashGpu {
    ...
    pub fn compute(&self, ctx: &GpuContext, images: &[Vec<u8>]) -> Result<Vec<u64>, GpuError> {
        ...
        let input_raw = self.buffer_pool.acquire(device, buffer_size, BufferUsage::Storage);
        let intermediate_raw = self.buffer_pool.acquire(device, buffer_size, BufferUsage::Storage);
        let output_raw = self.buffer_pool.acquire(device, buffer_size, BufferUsage::Storage);
        ...
        self.buffer_pool.release(intermediate_buffer.into_raw(), BufferUsage::Storage);
