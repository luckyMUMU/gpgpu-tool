...
impl Sha256Computer {
    ...
    pub fn with_workgroup_size(
        ...
        let pipeline_chained = ctx.get_or_create_pipeline(
            &PipelineDescriptor {
                ...
                wgsl: SHA256_CHAINED_WGSL.to_string(),
                workgroup_size,
                entry_point: "main",
    ...
    pub fn compute(
        ...
        dispatcher.dispatch_gpu(
            ctx,
            |ctx| self.compute_gpu(ctx, messages),
            || {
                #[cfg(feature = "cpu-fallback")]
                {
                    let cpu = Sha256Cpu::new();
                    cpu.compute(messages)
                }
    ...
    fn compute_gpu(
        ...
        if !single_block_indices.is_empty() {
            ...
            let batch_hashes = self.compute_single_block_batch(ctx, &batch_messages)?;
        ...
        if !multi_block_indices.is_empty() {
            ...
            let multi_hashes = self.compute_multi_block_batch(ctx, &multi_messages)?;
    ...
    pub fn batch_submitter<'a>(&'a self, ctx: &'a GpuContext) -> Result<Sha256BatchSubmitter<'a>, GpuError> {
        Ok(Sha256BatchSubmitter {
            computer: self,
            ctx,
            device: ctx.device()?,
            queue: ctx.queue()?,
    ...
    fn compute_multi_block_batch(
        ...
        // 前缀和：prefix_sums[i] = Σ(block_counts[0..i]) * 16，使着色器偏移查找 O(1)
        ...
        let prefix_sum_buffer = GpuBuffer::from_data(device, &prefix_sums, BufferUsage::Storage);
        ...
        self.pipeline_chained.dispatch(
            device, queue, &[&input_buffer, &output_buffer, &params_buffer, &prefix_sum_buffer], [dispatch_x, 1, 1],
            None,
            ctx.compute_units(),
        );
...
impl<'a> Sha256BatchSubmitter<'a> {
    ...
    fn submit_multi_block_message(
        ...
        // 前缀和：单消息时 prefix_sums[0] = 0
        let prefix_sum_buf = GpuBuffer::from_data(self.device, &[0u32], BufferUsage::Storage);
        ...
        self.computer.pipeline_chained.encode_dispatch_into(
            self.device, encoder, &[&input_gpu, &output_gpu, &params_buf, &prefix_sum_buf], [1, 1, 1],
            None,
            self.ctx.compute_units(),
        );
