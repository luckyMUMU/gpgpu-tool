...
fn test_block_hash_single_16x16() {
    ...
    let hasher = BlockHashComputer::with_config(&mut ctx, [256, 1, 1], HashSize::new(16)).expect("创建失败");
...
fn test_block_hash_single_32x32() {
    ...
    let hasher = BlockHashComputer::with_config(&mut ctx, [256, 1, 1], HashSize::new(32)).expect("创建失败");
