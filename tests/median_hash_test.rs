...
fn test_median_hash_single_16x16() {
    ...
    let hasher = MedianHashComputer::with_config(&mut ctx, [8, 8, 1], HashSize::new(16)).expect("创建失败");

    let image = test_data::gradient_image(256);
    let gpu_hashes = hasher.compute(&ctx, std::slice::from_ref(&image)).expect("计算失败");
    let cpu_hashes = hash_reference::median_hash_with_size(&image, 16, 16, 16);

    assert_eq!(gpu_hashes, cpu_hashes, "Median Hash 16x16 单图像测试失败");
}
